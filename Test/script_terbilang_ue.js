...
define(["N/record"], function(record) {
    ...
    function beforeSubmit(context) {
        ...
        if (context.type == context.UserEventType.CREATE || context.type == context.UserEventType.EDIT) {
            ...
            log.debug({
                "title": "start check: custbody_terbilang",
                "details": rec.getValue({
                    fieldId: 'custbody_ksi_terbilang'
                })
            });
            rec.setValue('custbody_terbilang', sayit.replace(/\s\s+/g, ' ').trim(), true);
            log.debug({
                "title": "end check: custbody_terbilang",
                "details": rec.getValue({
                    fieldId: 'custbody_terbilang'
                })
            });
        }
    }
