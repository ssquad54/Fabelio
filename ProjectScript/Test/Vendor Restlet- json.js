            ...
            try {
                ...
                for (var i = 0; i < countData; i++) {
                    ...
                    for (var key in dataKey) {
                        // START HEADER LEVEL
                        if ((key != 'addressbook') && dataKey.hasOwnProperty(key)) {
                            ...
                            if (key == 'vendorid') {
                                ...
                                if (dataCount == 1) {
                                    flagPrc = 'u';
                                    var currID = vendorSearch[0].getValue({
                                        name: "internalid"
                                    });
                                    ...
                                    recStr.push({
                                        'iter#': i,
                                        Duplicate_on_RecordID: objSearch[0].getValue("entityid")
                                    });
                                    break;
                                }
                                ...
                                log.debug({
                                    "title": "Check IF Exist: Klarifikasi Vendor " + dataCount,
                                    "details": objSearch
                                });
                                ...
                                objRecord.setValue({
                                    fieldId: 'ccustentity_ksi_klasifikasi_vendor',
                                    value: objID,
                                    ignoreFieldChange: true
                                });
                            } else if (key == 'nppkp') {
                            ...
                            for (var j = 0; j < arrKeys.length; j++) {
                                ...
                                var key2Notes = ['attention', 'addressee', 'addr1', 'addr2', 'addr3', 'addrphone', 'city', 'state', 'zip', 'country', 'override', 'addrtext'];
                                        ...
                                        if (dataKey2.hasOwnProperty(key2)) {
                                            if (key2 == 'label') {
                                                ...
                                                if (lineNumber >= 0) {
                                                    ...
                                                    log.debug({
                                                        "title": "Check IF Exist: Customer-AddressBookAddress-subAddr",
                                                        "details": subAddr
                                                    });
                                                } //
                                                else {
                    ...
                    if (flagPrc != '-' || flagPrc != 'e') {
                        ...
                        if (isPerson == 'T') {
                            recStr.push({
                                'iter#': currPos,
                                internalID: recLoad.getValue('internalid'),
                                use_form: recLoad.getText('customform'),
                                vendor_id: ksiVendID,
                                fullname: fullName,
                                company_name: recLoad.getValue('companyname'),
