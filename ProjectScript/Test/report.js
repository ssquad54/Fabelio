{
    ...
    var SCRIPT_BALANCESO_CS = 'customscript_mfg_balance_so_cs';
}
...
function balanceSOReport(request, response) {
	...
	*/
    ...
    try {
        ...
        var SOList = nlapiLoadSearch('transaction', 'customsearch_mfg_so_list');
        ...
        for (var i = 0; SOResults != null && i < SOResults.length; i++) {
            ...
            if (prepayList.length == 0) {
                ...
                for (var j = 0; j < prepayList.length; j++) {
                    ...
                    var bankCharges = prepayList[j].getValue('custbodybankcharges', null, 'max');
                    fTotalPrepayAmt += totalInv - bankCharges;

                    if (tranidInv == 'IV/MFG19000859') {
                        nlapiLogExecution('DEBUG', 'bankCharges = ' + bankCharges);
                    }
                    ...
                    sbl.setLineItemValue(COL_INVAMOUNT, nLineCtr, totalInv - bankCharges);
...
function getPrepayList(internalid) {
    ...
    returncols.push(new nlobjSearchColumn('custbodybankcharges', null, 'max'));
    returncols.push(new nlobjSearchColumn('amount', null, 'max'));
