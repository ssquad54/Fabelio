    ...
    function getData(context) {
        ...
        if (context.recordtype == 'vendorbill') {
            try {
                ...
                if (context.id && (context.id != '' || null != context.id)) {
                    arrFilters.push(search.createFilter({
                        name: 'internalid',
                        operator: search.Operator.IS,
                        values: context.id
                ...
                if (myVendBillList.length > 0) {
                    for (var i = 0; i < myVendBillList.length; i++) {
                        ...
                        var subListCount = recLoad.getLineCount({
                            sublistId: 'expense'
                        });
                        ...
                        for (var j = 0; j < subListCount; j++) {
                            sublistLine.push({
                                "line": recLoad.getSublistValue('expense', 'line', j),
                                "account": recLoad.getSublistText('expense', 'account', j),
                                "amount": recLoad.getSublistValue('expense', 'amount', j),
                                "taxcode": recLoad.getSublistText('expense', 'taxcode', j),
                                "memo": recLoad.getSublistValue('expense', 'memo', j),
                                "location": recLoad.getSublistText('expense', 'location', j)
                            });

                            log.debug("Sublist", sublistLine);
                        }

                        recStr.push({
                            "id": recLoad.getValue('id'),
                            "status": recLoad.getValue('status'),
                            "tranid": recLoad.getValue('tranid'),
                            "noPenerimaan": recLoad.getValue('custbody_ksi_no_penerimaan_barang'),
                            "vendorID": recLoad.getText('entity'),
                            "account": recLoad.getText('account'),
                            "terms": recLoad.getText('terms'),
                            "POtype": recLoad.getText('custbody_ksi_po_type'),
                            "klasifikasiPO": recLoad.getText('custbody_ksi_klasifikasi_po'),
                            "trandate": recLoad.getText('trandate'),
                            "tglFakturPajak": recLoad.getText('custbody_tgl_faktur_pajak'),
                            "noFakturPajak": recLoad.getValue('custbody_ksi_faktur_pajak'),
                            "memo": recLoad.getValue('memo'),
                            "total": recLoad.getValue('total'),
                            "expense": sublistLine
                        });
                ...
                for (var i = 0; i < countData; i++) {
                    ...
                    for (var key in dataKey) {
                        ...
                        if ((key != 'item' && key != 'expense') && dataKey.hasOwnProperty(key)) {
                            if (key == 'tranid') {
                                billNo = dataKey[key].trim();
                                var objSearch = search.create({
                                    type: search.Type.VENDOR_BILL,
                                    filters: ["tranid", search.Operator.IS, dataKey[key].trim()],
                                ...
                                if (dataCount >= 1) {
                                    ...
                                    recStr.push({
                                        ...
                                        "title": "Record Duplicated! Transaction ID: " + objSearch[0].getValue('tranid') + ".",
                                        "status": "Error",
                                        "o: errorCode": " "
                                    });
                                    return recStr;
                            ...
                            else if (key == 'POtype' && dataKey[key] != "") {
                                var objSearch = search.create({
                            ...
                            else if (key == 'klasifikasiPO' && dataKey[key] != "") {
                                var objSearch = search.create({
                            ...
                            else if (key == 'tglFakturPajak' && dataKey[key] != "") {
                                objRecord.setValue({
                            ...
                            else if (key == 'noFakturPajak' && dataKey[key] != "") {
                                objRecord.setValue({
                            ...
                            else if (key == 'noPenerimaan' && dataKey[key] != "") {
                                objRecord.setValue({
                    ...
                    recStr.push({
                        ...
                        "data": [{
                            ...
                            "tglFakturPajak": recLoad.getValue('custbody_tgl_faktur_pajak'),
                            "noFakturPajak": recLoad.getValue('custbody_ksi_faktur_pajak'),
                            "memo": recLoad.getValue('memo'),
                            "total": recLoad.getValue('total'),
                            "expense": sublistLine
